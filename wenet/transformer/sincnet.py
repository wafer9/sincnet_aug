...
class SincConv_fast(nn.Module):
    ...
    def __init__(self, out_channels=80, kernel_size=251, sample_rate=16000, in_channels=1,
                 stride=1, padding=125, dilation=1, bias=False, groups=1, min_low_hz=50, min_band_hz=50):
    ...
    def forward(self, waveforms):
        ...
        x = waveforms.transpose(1, 2) # (b, t, 1) -> (b, 1, t)
        x = F.conv1d(x, self.filters, stride=self.stride,
                        padding=self.padding, dilation=self.dilation,
                         bias=None, groups=1) 
        x = torch.abs(x)      # (b, 80, t)
        x = self.pool(x)
        x = torch.max(x, self.eps).log()
        x = x.transpose(1, 2) # (b, frame, 80)
        return x